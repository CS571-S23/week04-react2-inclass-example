import { useEffect, useState } from "react";
import President from "./President";

const WallOfPresidents = (props) => {

    const [presidents, setPresidents] = useState([]);

    const [newPresName, setNewPresName] = useState("");
    const [newPresParty, setNewPresParty] = useState("");

    function addPresident() {
        setPresidents([...presidents, {
            name: newPresName,
            political_party: newPresParty
        }]);
    }

    useEffect(() => {
        fetch('https://cs571.org/s23/week3/api/data', {
            headers: {
                "X-CS571-ID": "bid_00000000000000000000"
            }
        })
        .then(res => res.json())
        .then(prezData => {
            console.log(prezData);
            setPresidents(prezData);
        })
    }, [])

    return <div>
        <h1>Wall of Presidents</h1>
        <label htmlFor="inputPresName">Name</label>
        <input
            id="inputPresName"
            value={newPresName}
            onChange={(e) => setNewPresName(e.target.value)}
        />
        <label htmlFor="inputPresParty">Party</label>
        <input
            id="inputPresParty"
            value={newPresParty}
            onChange={(e) => setNewPresParty(e.target.value)}
        />
        <button onClick={addPresident}>Add President</button>
        {
            presidents.map(pres => <President key={pres.name} {...pres}/>)
        }
    </div>
}

export default WallOfPresidents;